...
def curriculum_learning(dsl_name="tinycalc", task_sequence: List[str] = None):
    """
    ...
    for i, task_id in enumerate(task_sequence):
        ...
        # Train on this task
        stats = trainer.train(
            num_episodes=1000, eval_every=200, save_every=500, verbose=True
        )

        results.append({"task_id": task_id, "stage": i + 1, "stats": stats})
...
def compare_curriculum_vs_single_task(dsl_name="tinycalc"):
    ...
    _, curriculum_agent = curriculum_learning(dsl_name, task_sequence)
    ...
    direct_agent = QLearningAgent(
        action_space_size=env.action_space_size, learning_rate=0.01, epsilon=0.5
    )

    trainer = RLTrainer(env, direct_agent, log_dir="output/rl_direct/tinycalc_005")
    _ = trainer.train(num_episodes=5000, eval_every=1000, verbose=True)

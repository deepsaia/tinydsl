"""
TinyCalc-specific evaluator with exact matching.

This is a thin wrapper around BaseEvaluator configured for TinyCalc
...
class TinyCalcEvaluator(BaseEvaluator):
    """
    Evaluates TinyCalc task outputs with exact matching.

    Uses BaseEvaluator with default exact comparison for unit conversion
    tasks where output format is predictable and standardized.
    """

    def __init__(self, benchmark_path: str):
        """
        Initialize TinyCalc evaluator with exact matching.

        Args:
            benchmark_path: Path to TinyCalc tasks JSON file
        """
    ...
    def evaluate_output(self, task_id: str, generated_output: str) -> Dict:
        ...
        Args:
            ...
            generated_output: Generated output from TinyCalc
...
if __name__ == "__main__":
    ...
    evaluator = TinyCalcEvaluator("tinycalc_tasks.json")

    # Example result from an LLM run
    results = [
        {"task_id": "tinycalc_001", "output": "5.0 grobble"},
        {"task_id": "tinycalc_002", "output": "10.5 flurb"},
    ]
